...
import { Upload, PlayCircle, Download, Moon, Sun, Settings, X, Library, Trash2, Edit2, Check, Save, Languages, Eye, EyeOff, ChevronLeft, ChevronDown, ChevronUp } from 'lucide-react';
...
interface AppViewProps {
  ...
  editingName: string;
  detailingQuiz: SavedQuiz | null;
  setDetailingQuiz: (quiz: SavedQuiz | null) => void;
  handleToggleQuestionVisibility: (quizId: string, index: number) => void;
...
export const AppView: React.FC<AppViewProps> = (props) => {
  // State to track which questions have their alternatives expanded in the details view
                          ...
                          {props.editingQuizId === quiz.id ? (
                            ...
                            <>
                              ...
                              <div className="flex items-center gap-0.5 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity ml-2">
                                <button onClick={() => { props.setDetailingQuiz(quiz); props.setIsSavedQuizzesOpen(false); }} className="p-2 text-slate-400 hover:text-indigo-500 rounded-lg" title={props.t.manageQuestions}><Eye className="w-4 h-4" /></button>
        ...
        {props.detailingQuiz ? (
          <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="flex items-center gap-4 mb-8">
              <button onClick={() => props.setDetailingQuiz(null)} className="p-2 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:text-indigo-600 transition-colors shadow-sm">
                <ChevronLeft className="w-6 h-6" />
              </button>
              <div>
                <h2 className="text-2xl font-bold text-slate-900 dark:text-white">{props.detailingQuiz.name}</h2>
                <p className="text-slate-500 dark:text-slate-400">{props.detailingQuiz.date} • {props.detailingQuiz.questions.length} {props.t.questionsLabel}</p>
              </div>
              <div className="ml-auto">
                <button onClick={() => props.loadSavedQuiz(props.detailingQuiz!)} className="px-6 py-2.5 bg-indigo-600 text-white font-semibold rounded-xl hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-500/20 flex items-center gap-2">
            ...
            <div className="space-y-3">
              <h3 className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider px-2">{props.t.quizDetails}</h3>
              {props.detailingQuiz.questions.map((q, idx) => {
                const isHidden = props.detailingQuiz?.hiddenIndices?.includes(idx);
                const isExpanded = expandedIndices.includes(idx);
                
                return (
                  <div key={idx} className={`flex flex-col p-4 rounded-2xl border transition-all ${isHidden ? 'bg-slate-100/50 dark:bg-slate-800/30 border-slate-200 dark:border-slate-800 opacity-60' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 shadow-sm'}`}>
                    <div className="flex items-start gap-4">
                      <button 
                        onClick={() => props.handleToggleQuestionVisibility(props.detailingQuiz!.id, idx)}
                        className={`mt-1 p-2 rounded-lg transition-colors ${isHidden ? 'text-slate-400 hover:text-emerald-500' : 'text-emerald-500 hover:text-slate-400'}`}
                        title={isHidden ? props.t.visible : props.t.hidden}
                      >
                        {isHidden ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                      </button>
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-[10px] font-bold uppercase tracking-tight px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-md">{q.topic}</span>
                          <span className="text-xs font-medium text-slate-400">#{idx + 1}</span>
                        </div>
                        <p className="text-slate-700 dark:text-slate-200 font-medium leading-relaxed">{q.question}</p>
                        ...
                        >
                          ...
                          {isExpanded ? props.t.hideOptions : props.t.showOptions}
                        </button>
                      </div>
                    </div>

                    {isExpanded && (
                      <div className="mt-4 ml-12 space-y-2 border-t border-slate-100 dark:border-slate-800 pt-4 animate-in fade-in zoom-in-95 duration-200">
                        {Object.keys(q.options).sort().map((key) => {
                          const isCorrect = key === q.correct.toLowerCase();
                          return (
                            <div 
                              key={key} 
                            ...
                            >
                              <span className={`w-5 h-5 rounded flex items-center justify-center text-[10px] font-bold uppercase shrink-0 ${isCorrect ? 'bg-emerald-500 text-white' : 'bg-slate-200 dark:bg-slate-700'}`}>
                                {key}
                              </span>
                              <span>{q.options[key]}</span>
                              {isCorrect && <Check className="w-4 h-4 ml-auto" />}
                            </div>
                          );
                        })}
                      </div>
