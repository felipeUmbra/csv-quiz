        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
                  ...
                  <div className="p-4">
                    <div className="space-y-4">
                      <label className="flex items-start gap-3 cursor-pointer group">
                        ...
                        <div>
                          <span className="text-base font-medium text-slate-900 dark:text-white block mb-1 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">Definir número de Questões</span>
                          <span className="text-sm text-slate-500 dark:text-slate-400">Permite escolher o número de perguntas antes de iniciar o teste.</span>
                        </div>
                      </label>
                      <label className="flex items-start gap-3 cursor-pointer group">
                        ...
                        <div>
                          <span className="text-base font-medium text-slate-900 dark:text-white block mb-1 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">Omitir resposta correta</span>
                          <span className="text-sm text-slate-500 dark:text-slate-400">Omite alternativa correta se você errar a questão.</span>
                        </div>
                      </label>
                    </div>
                  </div>
                  <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-b-2xl flex justify-end">
                    <button onClick={() => props.setIsSettingsOpen(false)} className="px-4 py-2 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 transition-colors text-sm">Concluir</button>
                  </div>
                </div>
              )}
              ...
              {props.isSavedQuizzesOpen && (
                <div ref={props.savedQuizzesPopoverRef} className="absolute top-full left-12 mt-2 w-80 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 origin-top-right">
                  <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    <h2 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2"><Library className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />Meus Quizzes</h2>
                          ...
                          {props.editingQuizId === quiz.id ? (
                            ...
                            <>
                              <button onClick={() => props.loadSavedQuiz(quiz)} className="flex-1 text-left min-w-0">
                                ...
                                <p className="text-xs text-slate-500 dark:text-slate-400">{quiz.date} • {quiz.questions.length} questões {typeof quiz.highestScore === 'number' ? ` • Recorde: ${quiz.highestScore}%` : ''}</p>
                              </button>
                              <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity ml-2">
                                <button onClick={() => { props.setEditingQuizId(quiz.id); props.setEditingName(quiz.name); }} className="p-2 text-slate-400 hover:text-indigo-500 rounded-lg"><Edit2 className="w-4 h-4" /></button>
                                <button onClick={() => props.deleteSavedQuiz(quiz.id)} className="p-2 text-slate-400 hover:text-red-500 rounded-lg"><Trash2 className="w-4 h-4" /></button>
                              </div>
                            </>
                          )}
                        </div>
            ...
            {(props.isStarted || props.isConfiguringQuestions) && (
              <button onClick={props.restartQuiz} className="text-sm font-medium text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors print:hidden">Sair do Quiz</button>
            )}
          </div>
        </div>
      </header>
        ...
        {props.pendingQuestions && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-950/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 w-full max-w-md overflow-hidden">
              ...
              <div className="p-6">
                <p className="text-slate-600 dark:text-slate-400 mb-6">O que você deseja fazer com as <strong>{props.pendingQuestions.length} questões</strong> encontradas?</p>
                <div className="grid gap-3">
                  <button onClick={() => props.handleUploadAction('run')} className="flex items-center justify-center gap-2 px-4 py-3 bg-indigo-600 text-white font-medium rounded-xl hover:bg-indigo-700 transition-colors"><PlayCircle className="w-5 h-5" />Apenas Iniciar</button>
                  <button onClick={() => props.handleUploadAction('both')} className="flex items-center justify-center gap-2 px-4 py-3 bg-emerald-600 text-white font-medium rounded-xl hover:bg-emerald-700 transition-colors"><Save className="w-5 h-5" />Salvar e Iniciar</button>
                  <button onClick={() => props.handleUploadAction('save')} className="flex items-center justify-center gap-2 px-4 py-3 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-medium rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"><Library className="w-5 h-5" />Apenas Salvar</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {props.isConfiguringQuestions ? (
          <div className="max-w-2xl mx-auto bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
              ...
              {Object.keys(props.topicCounts).map(topic => (
                <div key={topic} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
                  <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400">Total disponível: {props.topicCounts[topic]}</p>
                  </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-end border-t border-slate-100 dark:border-slate-800 pt-6">
              <button onClick={props.restartQuiz} className="px-6 py-3 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-medium rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors w-full sm:w-auto">Cancelar</button>
              <button onClick={props.startCustomQuiz} className="px-6 py-3 bg-indigo-600 text-white font-medium rounded-xl hover:bg-indigo-700 transition-colors w-full sm:w-auto">Iniciar Quiz</button>
            </div>
          </div>
        ) : !props.isStarted ? (
          <div className="max-w-2xl mx-auto text-center">
            <h2 className="text-4xl font-bold tracking-tight text-slate-900 dark:text-white sm:text-5xl mb-6">Transforme seus dados em conhecimento</h2>
            <p className="text-lg text-slate-600 dark:text-slate-400 mb-12">Faça upload de um arquivo CSV contendo perguntas e alternativas para gerar um questionário interativo instantaneamente.</p>
            <div className="grid sm:grid-cols-2 gap-6">
              <div className="relative group rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 p-8 hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all cursor-pointer flex flex-col items-center justify-center min-h-[240px]">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 text-center">Arraste e solte ou clique para selecionar seu arquivo.</p>
              </div>
              <div className="relative">
                <div ref={props.exampleButtonRef} onClick={() => props.setIsExamplePopoverOpen(!props.isExamplePopoverOpen)} className="rounded-2xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-8 hover:border-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-all cursor-pointer flex flex-col items-center justify-center min-h-[240px] w-full">
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 text-center">Escolha um dos nossos simulados prontos para testar.</p>
                </div>
            ...
            <div className="mt-12 text-left bg-slate-100 dark:bg-slate-800/50 rounded-xl p-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                ...
                <button onClick={props.downloadTemplate} className="inline-flex items-center text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 transition-colors bg-white dark:bg-slate-800 px-3 py-1.5 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700"><Download className="w-4 h-4 mr-2" />Baixar Modelo CSV</button>
              </div>
              <div className="overflow-x-auto">
                <table className="min-w-full text-sm text-slate-600 dark:text-slate-400">
                  ...
                  <tbody>
                    <tr><td className="pt-2">Matemática</td><td className="pt-2">Quanto é 2+2?</td><td className="pt-2">a</td><td className="pt-2">4</td><td className="pt-2">...</td></tr>
                  </tbody>
                </table>
              </div>
            </div>
